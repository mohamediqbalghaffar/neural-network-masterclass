(function() {
    ...
    NeuralApp.registerSection({
        ...
        contentAr: `
            <div class="topic-card accent-cyan">
                <h3><span class="card-icon">📈</span> من الانحدار الخطي إلى البيرسيبترون</h3>
                <p>إذا كنت مرتاحاً مع الانحدار الخطي، فأنت تفهم بالفعل أساسيات الشبكات العصبية. الخلية العصبية المفردة (البيرسيبترون) بدون دالة تنشيط متطابقة رياضياً مع نموذج الانحدار الخطي.</p>
                
                <div class="math-block"><span class="math-display">\\hat{y} = w_1x_1 + w_2x_2 + ... + w_nx_n + b = \\sum_{i=1}^{n} w_ix_i + b</span></div>
                
                <p>في مصطلحات التحليل، $w_i$ هي المعاملات (coefficients) و $b$ هو التقاطع (intercept). في التعلم العميق، $w_i$ هي <strong>الأوزان (weights)</strong> و $b$ هو <strong>الانحياز (bias)</strong>.</p>
            </div>
            
            <div class="topic-card accent-purple">
                <h3><span class="card-icon">🔌</span> الانحدار اللوجستي ودالة سيجمويد</h3>
                <p>يقدم الانحدار اللوجستي اللاخطية لتعيين التوقعات إلى احتمالات بين 0 و 1.</p>
                <p>تصبح الخلية العصبية نموذج انحدار لوجستي عندما نمرر ناتجها الخطي $z$ عبر دالة التنشيط سيجمويد $\\sigma(z)$.</p>
                <div class="math-block"><span class="math-display">\\sigma(z) = \\frac{1}{1 + e^{-z}}</span></div>
            </div>
            
            <div class="topic-card accent-pink">
                <h3><span class="card-icon">🛠️</span> هندسة الميزات مقابل التمثيلات المتعلمة</h3>
                <p>في التحليل التقليدي، يقضي علماء البيانات معظم وقتهم في صياغة الميزات (مثل الميزات متعددة الحدود) لتلتقط النماذج الخطية العلاقات المعقدة.</p>
                <p>تقوم الشبكات العصبية بأتمتة هذا الأمر. تعمل كل طبقة مخفية كمستخرج ميزات تلقائي، مما يخلق تمثيلات مجردة ومفيدة للبيانات الخام.</p>
                
                <div class="comparison-table">
                    <table>
                        <thead>
                            <tr>
                                <th>التحليل التقليدي (Analytics)</th>
                                <th>التعلم العميق (Deep Learning)</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td>هندسة ميزات يدوية</td>
                                <td>تعلم تمثيلي شامل (End-to-end)</td>
                            </tr>
                            <tr>
                                <td>معاملات قابلة للتفسير</td>
                                <td>أوزان صندوق أسود غير قابلة للتفسير</td>
                            </tr>
                            <tr>
                                <td>يعاني مع البيانات الخام (صور/صوت)</td>
                                <td>يتفوق في البيانات غير المنظمة</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="topic-card accent-amber">
                <h3><span class="card-icon">⚠️</span> عندما يفشل تعلم الآلة الكلاسيكي</h3>
                <p>تعاني النماذج الكلاسيكية مع مشكلة XOR - وهي عملية منطقية لاخطية بسيطة. حد القرار الخطي الواحد لا يمكنه فصل فئات XOR. إضافة طبقة مخفية واحدة فقط يمنح الشبكة القدرة على تعلم حدود لاخطية.</p>
                
                <div class="highlight-box tip">
                    <div class="highlight-title">ارتباط بالتحليل</div>
                    <p>فكر في الخلايا العصبية المخفية كشروط تفاعل مُتعلمة. بدلاً من تحديد $x_1 \\times x_2$ يدوياً، تتعلم الشبكة التفاعلات المفيدة للمهمة.</p>
                </div>
            </div>
            
            <div class="topic-card">
                <h3>عرض تفاعلي: ملاءمة اللاخطية</h3>
                <p>شاهد كيف أن إضافة خلايا عصبية مخفية يسمح للشبكة بملاءمة منحنى معقد، بينما النموذج الخطي يمكنه فقط رسم خط مستقيم.</p>
            ...
            <div class="key-takeaway">
                <h4>أهم النقاط</h4>
                <ul>
                    <li>الخلية العصبية الواحدة هي نموذج انحدار خطي؛ ومع سيجمويد، تصبح انحداراً لوجستياً.</li>
                    <li>يستبدل التعلم العميق هندسة الميزات اليدوية بالتعلم التمثيلي الهرمي.</li>
                    <li>توفر الطبقات المخفية اللاخطية اللازمة لحل المشاكل المعقدة مثل XOR.</li>
                </ul>
            </div>
        `,
